...
const validateNumber = (number) => {  //lo usaré para cantidad y edad
  if (!number) return false;
  // validación de longitud
  let lengthValid = number.length >= 1;

  // validación de formato
  let re = /^[0-9]+$/;
  let formatValid = re.test(number);

  // devolvemos la lógica AND de las validaciones.
  return lengthValid && formatValid;
};
const validateFechas = (fechas) => {
  if (!fechas) return false;

  // validación de formato
  let re = /^\d{4}-\d{2}-\d{2} \d{2}:\d{2}$/;
  let formatValid = re.test(fechas);
